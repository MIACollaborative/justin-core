...
describe('DataManager.updateItemInCollectionByUniquePropertyValue', () => {
  ...
  beforeEach(() => {
    ...
    dbStub = sinon.stub(MongoDBManager, 'updateItemInCollectionByUniqueProperty');
    checkInitStub = sinon.stub(dataManager, 'checkInitialization').callsFake(() => {});
  ...
  it('should update item and emit userUpdated for USERS collection', async () => {
    ...
    const result = await dataManager.updateItemInCollectionByUniquePropertyValue(
      'USERS',
      'email',
      'john@example.com',
      { name: 'John' }
    ...
    expect(emitSpy.calledWith('userUpdated', { email: 'john@example.com', name: 'John' })).toBe(true);
  });
  ...
  it('should handle error and call handleDbError', async () => {
    ...
    const handleDbErrorStub = sinon.stub(require('../data-manager.helpers'), 'handleDbError').returns(null);
    const result = await dataManager.updateItemInCollectionByUniquePropertyValue(
      'USERS',
      'email',
      'fail@example.com',
